...
function NewPatient() {
  ...
  const [form, setForm] = useState({
    full_name: "", cnic: "", phone: "", gender: "" as "" | "male" | "female" | "other",
    date_of_birth: "", blood_group: "", address: "", allergies: "", summary: "",
  });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <header>
        ...
        <p className="text-sm text-muted-foreground">Fill out at least CNIC or phone to register a patient.</p>
      </header>
      ...
      <Card className="card-shadow border-border/60 p-6 md:p-8">
        <form onSubmit={submit} className="grid gap-5 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="cnic" placeholder="35202-1234567-1" value={form.cnic} onChange={(e) => set("cnic", normalizeCnic(e.target.value))} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="ph">Phone</Label>
            <Input id="ph" placeholder="+923001234567" value={form.phone} onChange={(e) => set("phone", normalizePhone(e.target.value))} />
          </div>
          <div className="space-y-2">
            <Label>Gender</Label>
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Input id="al" placeholder="e.g. Penicillin, peanuts" value={form.allergies} onChange={(e) => set("allergies", e.target.value)} />
          </div>
          <div className="space-y-2 md:col-span-2">
            <Label htmlFor="sum">Medical history summary</Label>
            <Textarea id="sum" rows={3} value={form.summary} onChange={(e) => set("summary", e.target.value)} placeholder="Chronic conditions, surgeries, ongoing treatments…" />
          </div>
          <div className="md:col-span-2 flex justify-end gap-3 pt-2">
            <Button type="button" variant="outline" onClick={() => nav({ to: "/patients" })}>Cancel</Button>
